...
async def schedule_vc(bot, message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        yturl=""
        ysearch=""
        msg = await message.reply_text("⚡️ **Checking received input..**")
...
async def delete_all_sch(bot, message):
    ...
    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply("Do you want to cancel all the scheduled stream?ㅤㅤㅤㅤ ㅤ", reply_markup=reply_markup)
    await delete_messages([message])
